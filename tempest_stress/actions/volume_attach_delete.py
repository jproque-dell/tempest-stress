...
class VolumeAttachDeleteTest(stressaction.StressAction):
    ...
    def run(self):
        ...
        server = servers_client.create_server(name=vm_name,
                                              imageRef=self.image,
                                              flavorRef=self.flavor)['server']
        server_id = server['id']
        ...
        self.logger.info("attach volume (%s) to vm %s" %
                         (volume['id'], server_id))
        servers_client.attach_volume(server_id,
                                     volumeId=volume['id'],
                                     device='/dev/vdc')
        waiters.wait_for_volume_resource_status(volumes_client,
                                                volume['id'],
        ...
        waiters.wait_for_server_termination(servers_client,
                                            server_id)
